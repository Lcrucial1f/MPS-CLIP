...
from .GA import GatedGlobalAttention
...
class G2Adapter(nn.Module):
    """Gated Global Attention adapter described in the MPS-CLIP paper."""

    def __init__(self, hidden_size, bottleneck_dim=128, num_heads=8):
        super().__init__()
        self.down_proj = nn.Linear(hidden_size, bottleneck_dim)
        self.self_attention = nn.MultiheadAttention(bottleneck_dim, num_heads)
        self.attention_proj = nn.Linear(bottleneck_dim, bottleneck_dim)
        self.global_attention = GatedGlobalAttention(
            bottleneck_dim, bottleneck_dim, 0.1
        )
    ...
    def forward(self, x):
        ...
        x = self.global_attention(x.permute(1, 2, 0)).permute(2, 0, 1)
        x = torch.sigmoid(self.gate) * x
        return residual + self.up_proj(x)


def _build_g2_adapters(vision_cfg, text_cfg):
    vision_layers = vision_cfg.get('layers', 12) if isinstance(vision_cfg, dict) else vision_cfg.layers
    vision_width = vision_cfg.get('width', 768) if isinstance(vision_cfg, dict) else vision_cfg.width
    text_layers = text_cfg.get('layers', 12) if isinstance(text_cfg, dict) else text_cfg.layers
    text_width = text_cfg.get('width', 512) if isinstance(text_cfg, dict) else text_cfg.width

    vision_adapters = None
    if isinstance(vision_layers, int):
        vision_adapters = nn.ModuleList([
            G2Adapter(hidden_size=vision_width)
            for _ in range(vision_layers)
        ])

    text_adapters = nn.ModuleList([
        G2Adapter(hidden_size=text_width)
        for _ in range(text_layers)
    ])
    return vision_adapters, text_adapters
...
class CLIP(nn.Module):
    def __init__(
        ...
        super().__init__()
        vision_adapters, text_adapters = _build_g2_adapters(vision_cfg, text_cfg)
        self.visual = _build_vision_tower(
            embed_dim, vision_cfg, quick_gelu, cast_dtype,
            G2Adapter_img=vision_adapters,
        )

        text = _build_text_tower(
            embed_dim, text_cfg, quick_gelu, cast_dtype,
            G2Adapter_text=text_adapters,
        )
