...
def evaluation(model, data_loader, tokenizer, device, config):
    ...
    for image, regions, region_masks, img_id, num_regions in data_loader:
        ...
        regions = regions.to(device)          # [B,max_n,C,H,W]
        region_masks = region_masks.to(device)  # [B,max_n]

        if config['is_mps']:
            t1 = time.time()

            
            patch_feats, global_img_feat = model.get_vis_emb(image, seperate=True)
            # global_img_feat: [B,D]
    ...
    image_embeds = torch.cat(image_embeds, dim=0)  # [N_img,D]
    text_embeds = torch.cat(text_embeds, dim=0)    # [N_txt,D]
